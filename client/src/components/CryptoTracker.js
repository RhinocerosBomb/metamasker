...
function CryptoTracker() {
  ...
  const addCurrency = e => {
    updateCurrencyState({
      currencies: currencyState.currencies.concat(e.target.value)
    });
  };
  ...
  const updateCurrencies = selectedOptions => {
    ...
    if (currencyState.currencies.length > selectedOptions.length) {
      currencyState.currencies.forEach((currency, i) => {
        if (
          currency !== selectedOptions[i] &&
          currencyState.activeCurrency === currency
        ) {
          if (i === 0) {
  ...
  const switchTab = val => {
    if (
      val !== currencyState.activeCurrency &&
      val in data[cryptoCurrencies[0]]
    ) {
  ...
  const cryptoFilterOptions = ({ label }, query) => {
    return (
      label.toUpperCase().indexOf(query.toUpperCase()) >= 0 &&
      cryptoIndex++ < 100
    );
  };
        ...
        {data && (
          <React.Fragment>
            ...
            <div className="row tabRow">
              {currencyState.currencies.map(val => (
                <div
                  className={classNames({
                    tab: true,
                    active: val === currencyState.activeCurrency
                  })}
                  key={val}
              ...
              <span className="tab" onClick={() => setShowSearch('CURR')}>
                +
              </span>
            </div>
            <div className="row labelRow">
              ...
              <div className="cell">Change Today</div>
              <div className="cell">Mkt. Cap.</div>
              <div className="cell">Supply</div>
            </div>
            <div className="table">{renderRows()}</div>
            <LoadingBar
              height={10}
              start={liveMode}
              execute={fetchData}
            ...
            />
            <div
              className="addCoinButton"
              onClick={() => setShowSearch('CRYP')}
            >
              Add more coins
            </div>
          </React.Fragment>
        )}
        {!data && <img src={loader} style={{ margin: '10px' }} alt="loading" />}
      </div>
    );
  };
    ...
    for (let prop in data) {
      ...
      rowElems.push(
        <div className="row" key={prop}>
          <div className="cell">{prop}</div>
          <div className="cell">{rowData.PRICE}</div>
          <div className="cell">{rowData.CHANGEDAY}</div>
          <div className="cell">{rowData.MKTCAP}</div>
          <div className="cell">{rowData.SUPPLY}</div>
        </div>
      );
    }
  ...
  return (
    <div className="cryptoTracker">
      ...
      <SearchBar
        value={currencyState.currencies.map(val => ({
          value: val,
          label: val
        }))}
        options={currencySymbols.map(val => ({ value: val, label: val }))}
        onChange={selectedOptions =>
          updateCurrencies(selectedOptions.map(option => option.value))
        }
      ...
      <SearchBar
        ...
        onChange={selectedOptions =>
          setCryptoCurrencies(selectedOptions.map(option => option.value))
        }
        onKeyPress={e => enterKeyPress(e, addCurrency)}
